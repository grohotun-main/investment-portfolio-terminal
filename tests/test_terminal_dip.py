...
class TestParity(unittest.TestCase):
    """Cross-check the terminal view against what the Streamlit Buy-the-Dip tab
    renders: per card the 3 st.metric values (current drawdown / deeper-than /
    locked yield) and the forward st.dataframe cell-for-cell. The '1:1 exactly'
    gate on everything Streamlit exposes structurally. Slow (boots Streamlit)."""
    ...
    def setUpClass(cls):
        ...
        at.session_state["active_tab"] = "Buy the Dip"
        cls.at = at.run()
    ...
    def test_kpi_metrics_present(self):
        # Per-card POSITIONAL match (not set-membership): the dip body renders
        # cards in watch order and each _render_dip_card emits its 3 st.metrics
        # in order (current drawdown / deeper than / locked yield), so at.metric
        # is 3 metrics per card, grouped card-then-KPI in render order.
        # Set-membership under-gates when two cards share a KPI value — on the
        # synth fixture SCHD & SPY have identical current_dd (-3.2%) and
        # deeper_than (66% of history), so the 6 checks collapse to ~4 distinct
        # strings and a wrong per-card value equal to the other card's would slip
        # through. These 3 metrics are also the ONLY parity gate for the
        # drawdown-state / yield numbers (they aren't in the forward table), so
        # this is where per-card KPI parity must be tight. Same positional-scrape
        # idiom the Benchmark slice used.
        cards = {c["symbol"]: c for c in self.view["cards"]}
        # Streamlit renders cards in ITS watch order (trio-first-alpha = the
        # BUILD order); the terminal DISPLAY order is SPY-first since
        # 2026-07-19 — so pair Streamlit's positional trios to service cards
        # BY SYMBOL, keeping the per-card positional tightness intact.
        st_order = sorted(cards, key=lambda s: (s not in ds._TRIO, s))
        self.assertEqual(len(self.at.metric), 3 * len(cards),
                         "expected 3 st.metrics per card, card-then-KPI order")
        for i, sym in enumerate(st_order):
            card = cards[sym]
            trio = self.at.metric[3 * i: 3 * i + 3]
            for m, kpi in zip(trio, ("current_dd", "deeper_than", "locked_yield")):
                v = card["kpis"][kpi]["value"]
                self.assertEqual(m.value, v,
                                 f"{card['symbol']} {kpi}: Streamlit {m.value!r} "
                                 f"!= service {v!r}")

    def test_forward_tables_cell_for_cell(self):
        # Streamlit renders one forward st.dataframe per card; match by the
        # "If You Hold" column, then compare every cell to the service rows.
        dfs = {}
        for d in self.at.dataframe:
            cols = list(d.value.columns)
            if "If You Hold" in cols:
                key = tuple(d.value["If You Hold"].tolist())
                dfs[key] = d.value
        for card in self.view["cards"]:
            rows = card["forward_table"]["rows"]
            key = tuple(r["If You Hold"] for r in rows)
            self.assertIn(key, dfs,
                          f"no forward dataframe matching {card['symbol']} horizons")
            got = dfs[key].reset_index(drop=True)
            self.assertEqual(len(got), len(rows))
            for i, row in enumerate(rows):
                for col in card["forward_table"]["columns"]:
                    self.assertEqual(str(got[col].iloc[i]), row[col],
                                     f"{card['symbol']} row{i} col {col} mismatch")
