...
class TestParity(unittest.TestCase):
    """Cross-check the service's headline KPIs against the SAME numbers the
    Streamlit app renders, via AppTest over the committed fixture. The
    persistent KPI tape (render_chrome, app.py:9670-9709) renders cum TWR /
    annualized / IRR / max drawdown as st.markdown on EVERY tab, from the same
    `_portfolio_headline` source the Performance tiles use — so the unfiltered
    headline must appear byte-identical in the tape blob (no tab activation
    needed). Slow (boots Streamlit) — that is expected and intentional."""
    ...
    def test_cum_twr_matches_streamlit(self):
        self.assertIn(self.view["headline"][0]["value"], self.blob)

    def test_annualized_matches_streamlit(self):
        self.assertIn(self.view["headline"][1]["value"], self.blob)

    def test_max_dd_matches_streamlit(self):
        self.assertIn(self.view["headline"][3]["value"], self.blob)

    def test_irr_matches_streamlit(self):
        # On the fixture (no PORTFOLIO IRR row) this is "—"; the service must
        # agree with whatever the Streamlit tape renders.
        self.assertIn(self.view["headline"][2]["value"], self.blob)


class TestFilteredParity(unittest.TestCase):
    """The parity-critical cross-check: drive the Streamlit Account multiselect
    to a single bucket and confirm the service's FILTERED (daily-synthesized)
    cum TWR matches the refreshed tape. Skips the Streamlit half only if the
    filtered slice has no priceable holdings in the fixture."""
    ...
    def setUpClass(cls):
        ...
        from streamlit.testing.v1 import AppTest
        cls.frames = hs.load_frames(FIXTURE)
        # Pick one account filter id + its bucket label (deterministic: the
        # first sorted bucket, the same one app.py lists in the multiselect).
        snap_all = hs._current_snap(cls.frames)
        _, acct_by_id = hs._account_options(snap_all)
        cls.acct_id = next(iter(acct_by_id))
        cls.bucket = acct_by_id[cls.acct_id]
        cls.view = ps.build_performance_view(cls.frames, account=cls.acct_id)
        # Drive the "Account" multiselect to the single bucket. On Streamlit
        # 1.57 `Multiselect.set_value(...)` returns the widget and `.run()`
        # returns the REFRESHED AppTest (probed: the plan's `_tree` idiom is
        # wrong for this version — capture the return value of `.run()`).
        at = AppTest.from_file(str(ROOT / "app.py"), default_timeout=120).run()
        ms = [m for m in at.multiselect if m.label == "Account"]
        cls.drove_streamlit = bool(ms)
        if ms:
            at = ms[0].set_value([cls.bucket]).run()
        cls.blob = " ".join(str(getattr(e, "value", e)) for e in at.markdown)
    ...
    def test_filtered_cum_twr_matches_streamlit(self):
        # Filtered tape cum TWR = the synthesized value; the service must match.
        if self.view["meta"]["empty"]:
            self.skipTest("filtered slice has no priceable holdings in fixture")
        if not self.drove_streamlit:
            self.skipTest("Streamlit Account multiselect not found — cannot "
                          "drive the filtered tape")
        self.assertIn(self.view["headline"][0]["value"], self.blob)
